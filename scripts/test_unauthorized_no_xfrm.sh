...
echo "[4] send SPA for unauthorized service"
...
if [ "$SESSIONS_AFTER" != "$SESSIONS_BEFORE" ]; then
  echo "ERROR: unauthorized service changed PEP session count"
  exit 1
fi

if [ "$STATES_AFTER" != "$STATES_BEFORE" ]; then
  echo "ERROR: unauthorized service changed PEP XFRM states"
  docker exec cryptna-pep ip xfrm state
  exit 1
fi

if [ "$POLICIES_AFTER" != "$POLICIES_BEFORE" ]; then
  echo "ERROR: unauthorized service changed PEP XFRM policies"
  docker exec cryptna-pep ip xfrm policy
  exit 1
fi

echo "Unauthorized service no-XFRM test OK"
