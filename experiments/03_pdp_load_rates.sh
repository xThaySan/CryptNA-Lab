...
SUMMARY="$RESULT_DIR/03_pdp_load_rates_summary.csv"
...
for SCENARIO in $SCENARIOS; do
  for RATE in $RATES; do
    echo
    echo "[2] scenario=${SCENARIO} rate=${RATE} SPA/s n=${N}"

    docker compose up -d --force-recreate pdp pep client >/dev/null
    sleep 2

    SAFE_SCENARIO="$(echo "$SCENARIO" | tr '-' '_')"
    CONTAINER_OUT="/tmp/pdp_load_${SAFE_SCENARIO}_${RATE}.csv"
    RAW="$RESULT_DIR/03_pdp_load_rates_${SAFE_SCENARIO}_${RATE}sps.csv"
    ...
    python3 - <<'PY' "$RAW" "$SUMMARY" "$RATE" "$SCENARIO"
import csv
import statistics
import sys
...
print(
    f"{scenario} rate={rate} runs={line['runs']} ok={line['ok']} failed={line['failed']} "
